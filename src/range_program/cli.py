"""
CLI-точка входа Range Program.

Этот модуль создаёт Typer-приложение (команду `range`) и отвечает за запуск
интерактивного меню. Логика расчётов здесь не живёт: CLI лишь маршрутизирует
управление и аккуратно обрабатывает ошибки запуска меню.

Что делает:
- Инициализирует логирование (`setup_logging`) и логгер `cli`.
- Определяет поведение запуска:
  - `range` без подкоманды → запускает интерактивное меню по умолчанию.
  - `range menu` / `range ui` → запускает меню явно.
- При запуске меню:
  - пытается импортировать `range_program.menu` (может требовать доп. зависимостей),
  - создаёт сервисы (монеты/рынок/recalc/история/проверки),
  - при проблемах с зависимостями или локальными данными `data/*.json` печатает
    понятную подсказку и завершает процесс с кодом 1,
  - обрабатывает Ctrl+C как “нормальный выход”.
"""
