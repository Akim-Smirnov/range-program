...
# Сервисы создаются один раз на модуль, чтобы все команды работали с общим состоянием.
...
def _cli_root(ctx: typer.Context) -> None:
    """Логирует вызов подкоманды и аргументы CLI для трассировки."""
    if ctx.invoked_subcommand:
        log.info("invoke %s argv=%s", ctx.invoked_subcommand, " ".join(sys.argv[1:]))


def _handle_validation(e: ValidationError) -> None:
    """Печатает ошибку валидации и завершает команду с кодом 1."""
    typer.secho(str(e), fg=typer.colors.RED)
    raise typer.Exit(code=1)


def _handle_market(e: MarketDataError) -> None:
    """Печатает ошибку рыночных данных и завершает команду с кодом 1."""
    typer.secho(str(e), fg=typer.colors.RED)
    raise typer.Exit(code=1)


# CRUD-команды для локального списка монет.
@app.command("add")
def cmd_add(
...
def cmd_recalc(symbol: str = typer.Argument(..., help="Тикер монеты из хранилища")) -> None:
    ...
    # Блок сеток показываем только если у монеты заранее задан капитал.
...
def cmd_check(
    ...
    if all_coins and symbol:
        ...
        raise typer.Exit(code=1)
    if all_coins:
        # Массовая проверка: табличный вывод + агрегированная сводка по статусам.
        log.info("check --all")
...
def cmd_history(
    ...
    # История хранится отдельно: можно смотреть либо по символу, либо общий поток.
    if all_entries:
...
def cmd_backtest(
    ...
    # Утилита форматирует оценку длительности шага в днях по выбранному таймфрейму.
...
def cmd_suggest(
    ...
    # Команда ничего не сохраняет автоматически — только печатает рекомендацию.
    typer.echo(f"Best mode: {best.mode}")
    typer.echo("")
...
def _run_interactive_menu() -> None:
    ...
    # Явно прокидываем зависимости, чтобы menu-режим переиспользовал те же сервисы.
