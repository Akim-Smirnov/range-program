...
class CheckService:
    ...
    def run_check(self, symbol: str) -> CheckResult:
        ...
        try:
            ...
            raise ValidationError(f"Не удалось получить текущую цену: {e}") from e
    ...
    def run_check_safe(self, symbol: str) -> tuple[CheckResult | None, str | None]:
        ...
        try:
            return self.run_check(symbol), None
        except ValidationError as e:
            return None, str(e)

    def run_check_all(self) -> list[CheckTableRow]:
        ...
        for coin in self._coins.list_coins():
            res, err = self.run_check_safe(coin.symbol)
            if res is not None:
